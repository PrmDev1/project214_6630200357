...
const editSubject = async () => {
  ...
  try {
    const response = await fetch(`http://localhost:3000/subjects/${editingSubject.value.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
          ...
          <tbody>
            <tr v-for="subject in subjects" :key="subject.id" class="text-center">
              <td class="border border-gray-600 p-2">
                <input v-if="editingSubject && editingSubject.id === subject.id" v-model="editingSubject.id" class="w-full p-2 border border-gray-600 bg-gray-700 text-white" />
                <span v-else>{{ subject.id }}</span>
              </td>
              <td class="border border-gray-600 p-2">
                <input v-if="editingSubject && editingSubject.id === subject.id" v-model="editingSubject.name" class="w-full p-2 border border-gray-600 bg-gray-700 text-white" />
                <span v-else>{{ subject.name }}</span>
              </td>
              <td class="border border-gray-600 p-2">
                <select v-if="editingSubject && editingSubject.id === subject.id" v-model="editingSubject.credit" class="w-full p-2 border border-gray-600 bg-gray-700 text-white">
                  <option v-for="credit in creditOptions" :key="credit" :value="credit">{{ credit }}</option>
                </select>
                <span v-else>{{ subject.credit }}</span>
              </td>
              <td class="border border-gray-600 p-2">
                <select v-if="editingSubject && editingSubject.id === subject.id" v-model="editingSubject.grade" class="w-full p-2 border border-gray-600 bg-gray-700 text-white">
                  <option v-for="grade in gradeOptions" :key="grade" :value="grade">{{ grade }}</option>
                </select>
                <span v-else>{{ subject.grade }}</span>
              </td>
              <td class="border border-gray-600 p-2">
                <button v-if="editingSubject && editingSubject.id === subject.id" @click="editSubject" class="bg-blue-500 text-white px-4 py-2 rounded">Save</button>
                <button v-if="editingSubject && editingSubject.id === subject.id" @click="editingSubject = null" class="bg-gray-500 text-white px-4 py-2 rounded">Cancel</button>
                <button v-else @click="editingSubject = { ...subject }" class="bg-yellow-500 text-white px-4 py-2 rounded">Edit</button>
